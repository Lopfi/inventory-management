    ...
    <div v-if="location">
        <div id="location">
            ...
            <div id="location-image">
                <img :src="'http://localhost:3000/api/img/' + location.image" alt="couldn't load image" width="200" height="200" />
            </div>
...
export default {
    ...
    mounted() {
        axios.get(`http://localhost:3000/api/locations/${this.$route.params.id}`).then((response) => (this.location = response.data[0]));
        axios.get(`http://localhost:3000/api/locations/${this.$route.params.id}/items`).then((response) => (this.items = response.data));
    },
};
</script>
<style scoped>
